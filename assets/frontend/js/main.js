...
$(document).ready(function() {
    var $navbar = $(".navbar");
    var $mycss = $(".header-style-03.header-variant-08 .navbar-area .nav-container .logo-wrapper img");
    ...
    function AdjustHeader(){
        if ($(window).scrollTop() > 370) {
            if (!$navbar.hasClass("fixed")) {
                ...
                $mycss.css({"max-height" : "60px"});
                $setPadding.css({"top" : "0px"});
                $setColor.css({"color" : "#878a95"});
            }
            else{
                if ($navbar.hasClass("fixed")) {
                    $navbar.addClass("fixed");
            ...
            $setPadding.css({"top" : "-10px", "left" : "-40px"});
            $setColor.css({"color" : "#ffffffcc"});
        }
    }
});
